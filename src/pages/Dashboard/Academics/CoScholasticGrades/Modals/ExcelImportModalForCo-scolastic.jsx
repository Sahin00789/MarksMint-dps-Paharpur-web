...
const ExcelImportModalForCoScolastic = ({
  isOpen,
  onClose,
  selectedColumns = [],
  onImport,
    ...
    reader.onload = (e) => {
      try {
        ...
        const formattedData = rows.map((row, rowIndex) => {
          const item = { _row: rowIndex + 2 }; // +2 for 1-based index and header row
          
          headers.forEach((header, colIndex) => {
            const colName = requiredColumns.find(
              col => col.toLowerCase() === header.toLowerCase()
            );
            
            if (colName) {
              const value = row[colIndex];
              item[colName] = value !== undefined ? String(value).trim() : '';
              ...
              if ([
                ...
                const gradeValue = String(value).trim().toUpperCase();
                if (gradeValue && !gradeOptions.includes(gradeValue)) {
                  throw new Error(
                    `Row ${rowIndex + 2}: Invalid grade "${value}" in ${colName}. ` +
                    `Must be one of: ${gradeOptions.join(', ')}`
                  );
                }
              }
            }
          ...
          if (!item['Roll Number'] || !item['Student Name']) {
            throw new Error(
              `Row ${rowIndex + 2}: Missing required Roll Number or Student Name`
            );
          }
...
export default ExcelImportModalForCoScolastic;